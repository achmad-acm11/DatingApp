...
func (f queueRepository) getUserQueue(ctx *gin.Context, db *gorm.DB, userId int, gender string) []string {
	usersMatch := f.repoUser.GetAllTenMatch(ctx, db, userId, gender)
